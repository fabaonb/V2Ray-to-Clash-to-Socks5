        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const translations = {
                'zh': {
                    ...
                    clashPlaceholder: '在此处粘贴完整的 Clash 配置文件源文本\\n必须要包含 \`proxies:\` 节点数组部分...',
                ...
                'en': {
                    ...
                    clashPlaceholder: 'Paste your full Clash YAML configuration here.\\nIt MUST contain the \`proxies:\` proxy pool array...',
                    ...
                    try {
                        if (line.startsWith('vmess://')) {
                            ...
                            const proxy = {
                                ...
                                uuid: config.id, alterId: parseInt(config.aid || 0), cipher: "auto"
                            };
                            if (config.net === 'ws') {
                                proxy.network = 'ws';
                            ...
                            proxies.push(proxy);
                        } else if (line.startsWith('trojan://') || line.startsWith('vless://')) {
                            const url = new URL(line);
                            const proxy = {
                                name: decodeURIComponent(url.hash.slice(1)) || url.protocol.replace(':', '') + '-' + Math.random().toString(36).substr(2, 5),
                                type: url.protocol.replace(':', ''), server: url.hostname, port: parseInt(url.port),
                                password: decodeURIComponent(url.username || url.pathname.slice(2)),
                            };
                            if (proxy.type === 'vless') {
                                proxy.uuid = url.username; delete proxy.password;
                                if (url.searchParams.get('security') === 'tls') proxy.tls = true;
                                if (url.searchParams.get('type') === 'ws') {
                                    proxy.network = 'ws';
                                    proxy['ws-opts'] = { path: url.searchParams.get('path') || '/' };
                                }
                            }
                            if (url.searchParams.get('peer') || url.searchParams.get('sni')) {
                                proxy.sni = url.searchParams.get('peer') || url.searchParams.get('sni');
                            }
                            proxies.push(proxy);
                            ...
                            if (main.includes('@')) {
                                ...
                                const credentials = decodeBase64Safe(parts[0]).split(':');
                                method = credentials[0]; password = credentials[1];
                                const serverParts = parts[1].split(':');
                                server = serverParts[0]; port = parseInt(serverParts[1]);
                            } else {
                            ...
                            if (server && port) {
                                proxies.push({ name: hash || 'ss-' + Math.random().toString(36).substr(2, 5), type: 'ss', server: server, port: port, cipher: method, password: password });
                            }
                        }
                    } catch (e) {
            ...
            function doConvert() {
                ...
                if (inputStr) {
                    try {
                        const doc = jsyaml.load(inputStr);
